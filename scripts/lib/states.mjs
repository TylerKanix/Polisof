// FIPS -> { abbr, name }. Includes DC; excludes territories (no Senate seats).
export const STATES = {
...
export const ABBR_TO_FIPS = Object.fromEntries(
  Object.entries(STATES).map(([fips, s]) => [s.abbr, fips]),
);
export const NAME_TO_FIPS = Object.fromEntries(
  Object.entries(STATES).map(([fips, s]) => [s.name.toLowerCase(), fips]),
);
